...
For each console, retention keeps the newest `CONSOLE_<NAME>_RETENTION_DAILY` archives, then keeps at most `CONSOLE_<NAME>_RETENTION_WEEKLY` older archives, selecting one representative from each `CONSOLE_<NAME>_WEEKLY_INTERVAL` age bucket. Weekly representatives inside the daily window are already counted as daily archives, so they are not duplicated.
